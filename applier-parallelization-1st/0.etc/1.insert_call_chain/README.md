...
INSERT 워크로드의 콜체인을 분석한 자료. raw perf 데이터(`oncpu.data` 1.2GB, `oncpu.script` 163MB)는
용량 때문에 제외했고, **분석 산출물만** 담았다.

- 캡처: `-F 999`, dwarf call-graph, 전체 147,236 sample
- 진입 경로: 복제 적용 경로 (`slocator_repl_force`→`xlocator_repl_force`→…→`locator_insert_force`)

---

## 먼저 볼 것

👉 **`CALLCHAIN.md`** — 이 자료의 메인 문서. 캡처 정보 + 그림 + 상위 함수표 + 콜트리 + 관찰을 한 곳에 모았다.
...
| INSERT write 경로만 (locator_insert_force=100% 기준) 콜트리 | `analysis/calltree_root.txt` |
...
| INSERT write 경로만 flamegraph | `analysis/flame_locator_insert_force.svg` ※브라우저로 열기 |
| 콜체인을 노드-엣지 그래프 그림으로 | `analysis/callgraph.png` (벡터: `callgraph.svg`) |
| INSERT 경로에 등장한 **전 함수 평탄 목록** (컷오프 없음) | `analysis/inclusive_all.txt` |
| 캡처 **전체의 전 함수 평탄 목록** (1,840개, 컷오프 없음) | `analysis/inclusive_total.txt` |
| 재분석용 raw count (edge/inclusive) | `analysis/callgraph_data.json`, `fulltree_data.json` |

## 파일별 역할

### 문서
- **`CALLCHAIN.md`** — 메인. ① 캡처 정보 ② 그림 링크 ③ 상위 inclusive 함수표 ④ 관찰 ⑤ root=100% 콜트리 전문 ⑥ 전 함수표 ⑦ 전체(thread별) 콜체인 전문.

### 그림
- **`analysis/flame_full.svg`** — 전체 on-CPU flamegraph. 모든 스레드·함수. 브라우저로 열면 클릭 zoom / Ctrl+F search.
- **`analysis/flame_locator_insert_force.svg`** — `locator_insert_force` 포함 stack 만 추린 write-path flamegraph.
- **`analysis/callgraph.png` / `.svg`** — graphviz 노드-엣지 콜그래프 (root subtree, 노드 ≥0.5% root, 엣지 ≥30 sample). PNG는 문서/GitHub 인라인용, SVG는 확대용.

### 콜트리 (텍스트)
- **`analysis/calltree_root.txt`** — `locator_insert_force`=100% 기준 top-down 콜트리. INSERT write 경로 내부 구조에 집중. 노드 % = root 포함 sample 대비. (노드컷 0.1%, MIN_EDGE 30)
...
### 함수 평탄 표
- **`analysis/inclusive_all.txt`** — `locator_insert_force` 경로에 등장한 함수 inclusive %(분모=root sample). 컷오프 없음.
...
원본 측정 디렉토리에 있다: `bench_sampling/perf-runs/20260601-203144-cubserver-oncpu-config-parallel-poc-insert/`.
...
부모 % 보다 커 보일 수 있다(예: `log_append_undoredo_crumbs`). edge-local 비율이 아니라 **전역 inclusive 비중**으로 읽을 것.
