...
> 이 문서는 기능 코드를 보지 않고 스펙 자체가 완전하고 일관된 제품 계약인지 검토한 결과다. 현재 구현이 어떤 문제를 이미 처리하는지는 판정 기준으로 사용하지 않았다. 따라서 이 문서의 쟁점이 코드에 구현되어 있더라도 쟁점이 잘못 제기된 것은 아니며, 그 구현이 아래 계약과 수용 기준을 충족하는지는 별도의 코드 대조 문서인 [CODEX_SPEC_REVIEW_CUBRID_FIT_FILTER_FEATURE.md](CODEX_SPEC_REVIEW_CUBRID_FIT_FILTER_FEATURE.md)에서 확인해야 한다.
...
> **코드 대조 (2026-08-31, feature/CBRD-26246-develop@03162b928): 확정(지적이 코드에서 사실로 확인됨)**
>
> RK는 카탈로그 어디에도 저장되지 않는다. 마스터와 슬레이브가 각자, 매 DML마다 자기 인덱스 배열을 앞에서부터 훑어 처음 만나는 후보(PK이거나 모든 컬럼이 NOT NULL인 UNIQUE)를 그때그때 RK로 쓴다.
>
> - **배열 순서의 정체**: 제약 종류별 순서(FK → PK → UNIQUE → 일반 인덱스)는 코드에 고정돼 있고, 같은 종류 안에서는 제약이 만들어진 이력 순서다.
> - **결정적인 경우**: PK가 있으면 항상 PK. PK 없이 NOT NULL UNIQUE가 하나뿐이면 그것.
> - **흔들리는 경우**: PK 없이 NOT NULL UNIQUE가 여러 개면 어느 것이 RK인지가 생성 이력에 달린다. 복제로그에는 키 값만 실리고 "어떤 키로 골랐는지"는 안 실리므로, 슬레이브가 dump/load 재구축이나 제약 drop 후 재추가 등으로 다른 이력을 가지면 양쪽이 서로 다른 키를 RK로 잡을 수 있다.
> - **코드의 자인**: 소스의 TODO 주석이 "마스터에서 RK 이름을 받는 방식으로 교체 필요"를 명시하고 있다.
>
> 이 구조의 결정성은 "마스터와 슬레이브가 같은 DDL 이력을 밟는다"는 가정 위에서만 성립한다. HA 운영 중에는 DDL도 복제되므로 이 가정이 보통 성립하지만, 그렇다고 이 항목이 요구한 세 가지가 불필요해지는 것은 아니다.
>
> - **UK 정렬 규칙 명시·현재 선택 조회 수단**: 양쪽 일치와 무관한 운영 요구다. §4-2가 RK 컬럼의 ALTER를 제한하는데, 어느 키가 RK인지 조회할 수 없고 선택 규칙도 문서화돼 있지 않으면 운영자는 이력이 동일해도 자신이 지금 위험한 컬럼을 건드리는지 알 수 없다.
> - **선택 결과 영속**: 가정이 깨지는 경로가 실제 운영에 있다. unloaddb/loaddb로 슬레이브를 재구축하면(§8이 직접 다루는 경로) 덤프의 제약 출력 순서에 따라 카탈로그 이력이 달라질 수 있고, backup 복원·버전 업그레이드도 같은 위험을 반복한다. 이력이 최종적으로 같아져도, 재시작 catch-up이 RK를 바꾼 DDL 이전의 DML을 이미 새 카탈로그로 재해석하는 시점 창이 남는다.
>
> 결론: 요구된 세 항목(UK 정렬 규칙 명시, 선택 결과 영속, 현재 선택·후보 조회 수단)은 전부 미구현이다. 닫는 방향은 둘 중 하나다. 이력 동일 가정을 스펙의 계약으로 승격해 재구축·복원 경로의 보존 규칙을 명시하고 조회 수단을 제공하거나, 선택 결과를 영속해 가정 자체를 제거하거나. 관련: CF-01(로그에 RK 세대 정보 없음), CF-27(논리 RK ID 부재).
>
> 근거 위치: 선택 순회는 locator_sr.c:8040-8055(INSERT/DELETE)·8431-8447/8797(UPDATE)·btree.c:8264-8300(슬레이브 `btree_get_rkey_btid`), 후보 판정 `or_is_replication_candidate_key`, 배열 구성 `or_install_btids`(object_representation_sr.c), TODO 주석 object_representation_sr.c:4692.

> **추가 검증 (2026-08-31): "흔들리는 경우"는 실제로 얼마나 좁은가**
>
> 문답 1 — "ALTER가 양쪽에 동일하게 반영됐다면, 다시 배열을 훑어도 동일한 결과가 나오지 않는가?" 맞다. DDL은 문장 복제로 전파되므로 슬레이브도 같은 ALTER를 같은 스키마 매니저 경로로 실행하고, 제약이 property list의 같은 위치에 추가되어 배열 재구성도 결정적이다. 이력이 재생되는 정상 경로에서는 몇 번을 ALTER해도 불일치가 생기지 않는다.
>
> 문답 2 — "재구축도 결국 그 스키마로 생성한 DDL을 실행하는 것이니 순서가 또 같아야 하지 않는가?" 이것도 현재 구현 기준으로 맞다. 덤프의 UNIQUE 출력은 클래스 제약 리스트를 리스트 순서 그대로 돌면서 출력하고(unload_schema.c `emit_unique_def`), 그 리스트는 property list 저장 순서로 만들어지므로(class_object.c `classobj_make_class_constraints`), unloaddb가 만든 DDL에도 마스터의 제약 순서가 그대로 담긴다. 재구축 슬레이브도 같은 순서를 재현할 가능성이 높다(로드 시 한 문장 안 다중 제약의 처리 순서는 미확인).
>
> 따라서 실제로 남는 위험은 다음 둘로 좁혀진다.
>
> - **순서 보존이 계약이 아니라 우연이라는 것**: 위 보존은 어떤 문서에도 약속돼 있지 않은 구현 세부다. 스키마 파일을 사람이 편집·정렬하거나(포맷터, diff 병합), 버전 업그레이드에서 덤프 형식·카탈로그 마이그레이션이 바뀌면 조용히 깨지고, 깨졌는지 감지할 수단(조회 API)도 없다. RK 정확성이 이 우연에 걸려 있다는 것 자체가 CF-02가 지적하는 바다.
> - **적용 시점 재계산 창**: 순서가 완벽히 보존돼도 재계산은 "적용하는 순간의" 카탈로그 기준이다. RK가 UNIQUE(a)이던 시절의 로그(키 값 = a의 값)를 ADD PRIMARY KEY(b) 반영 이후에 재시작 catch-up이 재적용하면, 슬레이브는 PK(b) 인덱스를 a의 값으로 검색한다. 대개는 못 찾아 skip이지만 타입이 호환되면 다른 행이 매칭될 구조적 가능성이 있다. 순차 적용 중에는 로그 순서가 DDL 장벽을 지켜주지만, 재적용과 병렬 적용에서는 이 창이 열린다. (코드 구조에서 확인한 추론이며 실측은 아직이다.)
...
- **`CODEX_SPEC_REVIEW_CUBRID_FIT_FILTER_FEATURE.md`:** 현재 feature 코드가 해당 경로를 구현했는지, develop에도 있던 기존 문제인지, CUBRID 범위 밖인지 판단한다.
