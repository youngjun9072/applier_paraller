---
tags: [writeset, mysql, 대조실험, 벤치마크, 보고서]
date: 2026-08-26
status: 실측·소스분석 완료
---

# MySQL 9.7 FK 병렬복제 대조 실험 보고서

> [!summary] 한 문단 요약
> CUBRID writeset v2의 FK 설계가 MySQL 대비 어떤 이점을 갖는지 확인하기 위해, MySQL 9.7 복제 환경에서 "같은 부모를 참조하는 자식 트랜잭션들(형제)이 병렬 적용되는가"를 실측했다. 결과: **자식들이 부모의 서로 다른 행을 참조하면 병렬, 같은 행을 참조하면 직렬로 강제**된다. 직렬의 원인은 마스터가 binlog에 기록하는 의존성 값(last_committed)이 형제끼리 사슬로 묶이기 때문이며, 이는 MySQL이 5년째 방치 중인 과직렬화 버그 #110071의 실체다.
